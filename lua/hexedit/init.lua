-- Original source: https://github.com/ArcaneSpecs/HexEditor.nvim
local u = require 'hexedit.utils'
...
M.config = {
  ...
  is_file_binary_pre_read = function()
    local binary_ext = { 'gb', 'gbc', 'out', 'bin', 'png', 'jpg', 'jpeg', 'exe', 'dll', 'pak', 'so', 'a' }
...
M.setup = function(options)
  ...
  if not u.is_program_executable(dump_program) then
    return
  end
  if not u.is_program_executable(assemble_program) then
    return
  end
